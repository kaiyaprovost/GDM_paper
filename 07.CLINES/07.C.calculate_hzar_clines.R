...
setwd("~")
## take in arguments
species="bellii" ## change as needed
plotWholeSpecies=T
doFull=F
desert="CHI" ## change as needed
## set up functions
...
all_qopt=read.table("AllSpeciesMetadata_allK_5january2021.csv",
                      sep=",",header=T)
all_qopt = all_qopt[order(all_qopt$RG),]
...
files=list.files(pattern="1.admix.Q.npy.csv$",full.names = T,recursive=T)
files2=list.files(pattern="NGSadmix.qopt$",full.names = T,recursive=T)
files = unique(c(files,files2))
## subset based on what you want
files = files[(grepl(desert,files))]
files = sample(files)
for(species in rev(c(#"bellii",
                     "bellii-NOWEIRD",
...
final_data=read.table("ALLSPECIES_hzar_chroms.txt", sep="\t",header=T)
