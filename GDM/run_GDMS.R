...
generateMatrixCsvString = function(matVariable,doPCA=F,doGENE=F,doChroms=F,spp,whichpca,whichchrom,verbose=F,
                                   basestr="/Users/kprovost/Dropbox (AMNH)/Dissertation/CHAPTER3_TRAITS/Distances/BY_SPECIES/CSVS/",
  ...
  if(matVariable=="STR") {
    ...
    if (doPCA == F) {
      if (doGENE == F) {
        ...
        ## do genes is true
        if (doChroms==F) {
          csvstring = paste(basegene,spp,"/",spp,"_distancematrix_FULLGENOME.csv.converted",sep = "")#[1]
          ## "BELLII_distancematrix_FULLGENOME.csv.converted"
        } else {
          csvstring = list.files(path=paste(basegene,spp,"/",sep=""),pattern=paste("Tgut_",whichchrom,".csv.converted$",sep=""),full.names = T)#[1]
        }
      }
    } else {
